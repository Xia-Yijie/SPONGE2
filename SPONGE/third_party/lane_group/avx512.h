...
struct LaneGroup
{
    ...
    __host__ __device__ __forceinline__ static LaneMask Ballot(__mmask16 predicate)
    {
        return LaneMask(static_cast<unsigned int>(predicate));
    }

    __host__ __device__ __forceinline__ static LaneMask Ballot(__mmask8 predicate)
    {
        return LaneMask(static_cast<unsigned int>(predicate));
    }
    ...
    __host__ __device__ __forceinline__ static float Reduce_Sum(__m512 value)
    {
    ...
    __host__ __device__ __forceinline__ static double Reduce_Sum(__m512d value)
    {
        alignas(64) double lanes[8];
        _mm512_storeu_pd(lanes, value);
        double sum = 0.0;
        for (int i = 0; i < 8; ++i)
        {
            sum += lanes[i];
