...
// Rys quadrature Fock kernel for d-containing shell quartets (l_max <= 2).
    ...
    int shell_buf_size, float prim_screen_tol)
{
    SIMPLE_DEVICE_FOR(task_id, n_tasks)
    {
            ...
            shell_screen_tol)
        {
            ...
            const int nrys = (L_sum + 2) / 2;  // ceil((L_sum+1)/2)
            ...
            float eri_cart[1296];  // max 6^4 for dddd
                ...
                for (int jp = 0; jp < np[1]; jp++)
                {
                    ...
                    for (int kp = 0; kp < np[2]; kp++)
                    {
                        ...
                        for (int lp = 0; lp < np[3]; lp++)
                        {
                            ...
                            double F_boys[10];  // max 2*5=10 moments
                            eri_boys(F_boys, (float)T, 2 * nrys - 1);
                            double rys_r[5], rys_w[5];
                            rys_roots_weights(nrys, (double)T, rys_r, rys_w);

                            // For each Rys root: build Gx, Gy, Gz via VRR, then
                            // HRR to get ERI
                            for (int ir = 0; ir < nrys; ir++)
                            {
                                ...
                                // HRR + assembly: for each Cartesian component,
                                // extract the ERI
                                int idx = 0;
                                    ...
                                    for (int c1 = 0; c1 < dim_cart[1]; c1++)
                                    {
                                        ...
                                        for (int c2 = 0; c2 < dim_cart[2]; c2++)
                                        {
                                                 ...
                                                 c3++)
                                            {
                                                ...
                                                // Ket HRR: extract column at
                                                // (kl) index = ax2+ax3 Then
                                                // distribute to (ax2, ax3) Wait
                                                // - for ket HRR we need
                                                // G[ij_final][j] for j=0..kl_am
                                                // But we already extracted at
                                                // j=ax2+ax3. We need to do ket
                                                // HRR too.

                                                // Actually, let me redo:
                                                // extract G[i][j] for needed ij
                                                // and all j, do bra HRR on ij,
                                                // then ket HRR on kl.

                                                // Bra HRR for x: from
                                                // G[0..ij_am][kl_j] → I(ax0,
                                                // ax1, kl_j) Then Ket HRR: from
                                                // I(ax0, ax1, 0..kl_am) →
                                                // I(ax0, ax1, ax2, ax3)

                                                // Recompute properly:
                                                float
                                                    gx_bra[5];  // result after
                                                                // bra HRR,
                                                                // indexed by kl
