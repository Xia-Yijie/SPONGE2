    ...
    int shell_buf_size, float prim_screen_tol)
{
    ...
    const int l[4] = {_2S2P_IS_P(0), _2S2P_IS_P(1), _2S2P_IS_P(2), _2S2P_IS_P(3)};
    const int dim_c[4] = {_2S2P_IS_P(0)?3:1, _2S2P_IS_P(1)?3:1, _2S2P_IS_P(2)?3:1, _2S2P_IS_P(3)?3:1};
    const int dim_e[4] = {dim_c[0], dim_c[1], dim_c[2], dim_c[3]}; // p: cart==sph==3
    ...
    SIMPLE_DEVICE_FOR(task_id, n_tasks)
    {
        ...
        if (is_spherical)
        {
            for (int si = 0; si < 4; si++)
            {
                if (l[si] == 0)
                {
                    const float c2s = cart2sph_mat[ao_offsets_cart[sh[si]]*nao_sph + off[si]];
                        ...
                        for (int inn = 0; inn < stride; inn++) {
                            ...
                            for (int cc = 0; cc < 3; cc++)
                                sum += (double)cart2sph_mat[(oc+cc)*nao_sph+(os+s)] *
                                       (double)tmp[o*3*stride + cc*stride + inn];
                            eri_cart[o*3*stride + s*stride + inn] = (float)sum;
                        }
            }
        }
        ...
        const bool jk_same_braket = (tk.x == tk.z && tk.y == tk.w);
        {
            ...
            if (jk_same_bra && q > p) { idx += dim_e[2]*dim_e[3]; continue; }
            for (int c2 = 0; c2 < dim_e[2]; c2++) {
