...
#if P_POS == 0
                        const float sx = Px - RC[0][0], sy = Py - RC[0][1], sz = Pz - RC[0][2];
                        const float rcoeff = inv2p;
#elif P_POS == 1
                        const float sx = Px - RC[1][0], sy = Py - RC[1][1], sz = Pz - RC[1][2];
                        const float rcoeff = inv2p;
#elif P_POS == 2
                        const float sx = Qx - RC[2][0], sy = Qy - RC[2][1], sz = Qz - RC[2][2];
                        const float rcoeff = -inv2q;
#else // P_POS == 3
                        const float sx = Qx - RC[3][0], sy = Qy - RC[3][1], sz = Qz - RC[3][2];
                        const float rcoeff = -inv2q;
#endif
...
#if P_POS == 0
            const int ao_p = off[0] + d, ao_q = off[1], ao_r = off[2], ao_s = off[3];
#elif P_POS == 1
            const int ao_p = off[0], ao_q = off[1] + d, ao_r = off[2], ao_s = off[3];
#elif P_POS == 2
            const int ao_p = off[0], ao_q = off[1], ao_r = off[2] + d, ao_s = off[3];
#else
            const int ao_p = off[0], ao_q = off[1], ao_r = off[2], ao_s = off[3] + d;
#endif
