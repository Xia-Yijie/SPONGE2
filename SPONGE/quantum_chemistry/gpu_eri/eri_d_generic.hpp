...
// Generic register-only Fock kernel for d-containing shell quartets (l_max <=
// 2). Runtime L_sum (2..8), uses eri_common utilities with d-shell extensions.
// No scratch buffers. Used as fallback for all pair-type combos involving d
// shells.
    ...
    int shell_buf_size, float prim_screen_tol)
{
    SIMPLE_DEVICE_FOR(task_id, n_tasks)
    {
            ...
            shell_screen_tol)
        {
            ...
            // Max for l<=2: 6^4=1296 cart, 5^4=625 sph
            float eri_cart[1296];
            ...
            if (is_spherical)
            {
                ...
                {
                    const int nc = dim_cart[si];
                    const int ns = dim_eff[si];
                    if (nc == ns && l[si] == 0)
                    {
                        float c2s =
