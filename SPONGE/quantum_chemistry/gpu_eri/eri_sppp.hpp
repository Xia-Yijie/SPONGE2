...
// Register-only (sppp)-type Fock kernel. L_sum=3, needs Boys F0-F3.
// Three of the four shells are p (l=1), one is s (l=0).
            ...
            for (int jp = 0; jp < np[1]; jp++)
            {
                ...
                for (int kp = 0; kp < np[2]; kp++)
                {
                    ...
                    for (int lp = 0; lp < np[3]; lp++)
                    {
                        ...
                        // R^0 tensor (20 elements for L=3, workspace 35)
                        float R0[20], Rw[35];
