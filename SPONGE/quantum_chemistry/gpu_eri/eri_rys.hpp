...
// Rys quadrature engine: roots/weights via Chebyshev interpolation (from gpu4pyscf),
// VRR for 2D integrals, HRR for angular momentum distribution.
//
...
// Output: roots[nrys] = t_i^2, weights[nrys]
// Satisfies: sum_i w_i * root_i^k = F_k(T)
static __device__ void rys_roots_weights(
    int nrys, double T, double* roots, double* weights)
{
    if (T < 3.e-7)
    {
    ...
    const double *datax = RYS_RW_DATA + RYS_DEGREE1 * RYS_INTERVALS * nrys * (nrys - 1);
    int it = (int)(T * 0.4);
...
static __device__ void rys_vrr_2d(
    float* G, int ij_max, int kl_max, int g_stride,
    float Cx_bra, float Cx_ket, float B00, float B10, float B01)
{
    G[0] = 1.0f;

    // Vertical on bra: G[i+1][0]
    for (int i = 0; i < ij_max; i++)
        G[(i+1)*g_stride] = Cx_bra * G[i*g_stride] +
...
static __device__ float rys_hrr_1d(
    const float* g_col, int la, int lb, float AB)
{
