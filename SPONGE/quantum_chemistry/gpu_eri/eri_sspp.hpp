...
// Register-only (sspp)-type Fock kernel. L_sum=2, needs Boys F0-F2.
// Two of the four shells are p (l=1), two are s (l=0).
    ...
    int shell_buf_size, float prim_screen_tol)
{
    SIMPLE_DEVICE_FOR(task_id, n_tasks)
    {
        ...
        // Max 3^4=81 components, but for sspp it's dim[0]*dim[1]*dim[2]*dim[3]
        ...
        for (int ip = 0; ip < np[0]; ip++)
        {
            ...
            for (int jp = 0; jp < np[1]; jp++)
            {
                ...
                const float PB[3] = {Px - RC[1][0], Py - RC[1][1], Pz - RC[1][2]};
                ...
                for (int kp = 0; kp < np[2]; kp++)
                {
                    ...
                    for (int lp = 0; lp < np[3]; lp++)
                    {
                        ...
                        const float QCv[3] = {Qx - RC[2][0], Qy - RC[2][1], Qz - RC[2][2]};
                        const float QD[3] = {Qx - RC[3][0], Qy - RC[3][1], Qz - RC[3][2]};
                        ...
                        // R^0 tensor (10 elements for L=2, workspace 15)
                        float R0[10], Rw[15];
                        sp_build_R0(R0, Rw, F_boys, alpha, PQ, 2);
        ...
        if (is_spherical)
        {
            for (int si = 0; si < 4; si++)
            {
                ...
                // flat_idx = c0 * stride0 + c1 * stride1 + c2 * stride2 + c3
                // stride_i = product of dim[j] for j > i
                int stride[4];
                stride[3] = 1;
                stride[2] = dim[3];
                ...
                // For each element, decompose flat index to get the component
                // along axis si, transform it
                const int outer_stride = stride[si] * 3; // stride for the axis above si
                const int inner_stride = stride[si]; // stride for axis si itself
                const int n_outer = (si == 0) ? 1 : n_cart / (dim[si] * inner_stride);

                // Transform: for each (outer_block, sph_idx, inner_idx)
                for (int o = 0; o < n_cart / (3 * inner_stride); o++)
                {
                    const int block_start = (o / (n_cart / (outer_stride > 0 ? outer_stride : 1))) * outer_stride +
                                            (o % (inner_stride > 0 ? (n_cart / outer_stride > 0 ? 1 : 1) : 1));
                    // This is getting complicated. Let me use a simpler approach.
                }
                // Simpler: iterate over all indices, and for axis si, do the transform
                for (int flat = 0; flat < n_cart; flat++)
                {
                    // Decompose flat index to (c0,c1,c2,c3)
                    int rem = flat;
                    int c[4];
                    c[0] = rem / stride[0]; rem %= stride[0];
                    c[1] = rem / stride[1]; rem %= stride[1];
                    c[2] = rem / stride[2]; rem %= stride[2];
                    c[3] = rem;

                    if (c[si] == 0) // only compute once per output position
                    {
                        for (int s = 0; s < 3; s++)
                        {
                            double sum = 0.0;
                            ...
                            {
                                int idx_in = flat + cc * stride[si]; // replace c[si]=0 with cc
...

        {
            ...
            for (int c0 = 0; c0 < dim[0]; c0++)
            {
                ...
                for (int c1 = 0; c1 < dim[1]; c1++)
                {
                    ...
                    if (jk_same_bra && q > p) { idx += dim[2] * dim[3]; continue; }
                    for (int c2 = 0; c2 < dim[2]; c2++)
                    {
