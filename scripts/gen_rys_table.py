...
def rys_roots_from_moments(nrys, T):
    ...
    """
    T = float(T)
    moments = np.array([boys_function(k, np.array([T]))[0] for k in range(2*nrys)])
...
def generate_tables():
    ...
    for nrys in range(1, MAX_NRYS + 1):
        ...
        for interval_idx in range(N_INTERVALS):
            ...
            for i in range(nrys):
                ...
                all_cheb_coeffs.append((nrys, 2*i+1, interval_idx, coeffs_w))
    ...
    for nrys in range(1, MAX_NRYS + 1):
        ...
        print(f"  nrys={nrys}: max_err_root={max_err_r:.2e}, max_err_weight={max_err_w:.2e}")

    return smallx_r0, smallx_r1, smallx_w0, smallx_w1, largex_r, largex_w, all_cheb_coeffs

def write_header(filename, smallx_r0, smallx_r1, smallx_w0, smallx_w1,
                 largex_r, largex_w, all_cheb_coeffs):
    ...
    with open(filename, 'w') as f:
        ...
        f.write(f"// Chebyshev degree = {DEGREE}, intervals = {N_INTERVALS}, "
            ...
            for rw_idx in range(2 * nrys):
                for deg in range(DEGREE + 1):
                    for intv in range(N_INTERVALS):
                        ...
                        val = 0.0
                        for entry in all_cheb_coeffs:
                            if entry[0] == nrys and entry[1] == rw_idx and entry[2] == intv:
                                coeffs = entry[3]
        ...
        print(f"Written {filename}: {len(rw_data)} RW coefficients, "
